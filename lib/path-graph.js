...
var findClosest = function (points, position, fillGaps) {
  var result = points.reduce(function (prev, point) {
...
PathGraph.prototype._fillGapsAndOptimize = function () {
  var newSegs = this._edges.map(function (x) { return x.segment })
  this._edges = []
  this.length = 0
  newSegs.forEach(function (newSeg, index) {
    var start
    var end
    var fillGaps = this._fillGaps
    var otherSegs = newSegs.slice(0, index).concat(newSegs.slice(index + 1))
    var otherPoints = otherSegs.reduce(function (prev, seg) {
      return prev.concat([{position: seg.start, edges: []}, {position: seg.end, edges: []}])
    }, [])
    ...
    var existing = find(edges, function (edge) {
